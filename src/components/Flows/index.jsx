...
const INIT_NODES = [
  ...
  { id:'n3', type:'options',  x:310, y:290, data:{ text:'Elegí una opción:', options:['Ver productos','Soporte','Agente'] } },
  { id:'n4', type:'message',  x:580, y:100, data:{ text:'Te muestro el catálogo.' } },
  { id:'n5', type:'agent',    x:580, y:290, data:{ message:'Un agente te atenderá.' } },
...
function Bez({ from, to, color, thick, onClick }) {
  ...
  return <g onClick={onClick} style={{cursor:onClick?'pointer':'default'}}><path d={d} fill="none" stroke="transparent" strokeWidth={14}/><path d={d} fill="none" stroke={color} strokeWidth={thick?2.5:1.8} opacity={thick?1:0.7} style={{filter:thick?`drop-shadow(0 0 3px ${color})`:'none'}}/><circle cx={to.x} cy={to.y} r={3.5} fill={color}/></g>
}
...
function FlowNode({ node, selected, mode, onMD, onPD, onPU, isCT }) {
  ...
  const sh=selected?`drop-shadow(0 0 8px ${meta.color}55)`:`drop-shadow(0 1px 4px rgba(0,0,0,${L?0.08:0.3}))`
  ...
  return (
    <g transform={`translate(${node.x},${node.y})`} style={{cursor:'grab',userSelect:'none'}} onMouseDown={e=>{e.stopPropagation();onMD(e)}}>
      {selected&&<rect x={-3} y={-3} width={NW+6} height={h+6} rx={13} fill={meta.color+'12'}/>}
      <rect x={0} y={0} width={NW} height={h} rx={11} fill={bg} stroke={bd} strokeWidth={selected?2:1} style={{filter:sh,transition:'all 0.15s'}}/>
      ...
      {node.type==='agent'&&<foreignObject x={8} y={34} width={NW-16} height={34}><div xmlns="http://www.w3.org/1999/xhtml" style={{fontSize:10,color:mu,lineHeight:1.4,overflow:'hidden',display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical'}}>{node.data.message}</div></foreignObject>}
      {node.type==='delay'&&<text x={10} y={52} fontSize={10} fill={mu} fontFamily="system-ui">Espera: {node.data.seconds}s</text>}
      {node.type==='input'&&<foreignObject x={8} y={34} width={NW-16} height={34}><div xmlns="http://www.w3.org/1999/xhtml" style={{fontSize:10,color:mu,lineHeight:1.4,overflow:'hidden',display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical'}}>{node.data.question}</div></foreignObject>}
      ...
      {node.type!=='trigger'&&<g onMouseUp={e=>{e.stopPropagation();onPU('in')}}><circle cx={0} cy={h/2} r={9} fill="transparent" style={{cursor:'crosshair'}}/><circle cx={0} cy={h/2} r={5.5} fill={bg} stroke={isCT?'#0EA5E9':bd} strokeWidth={1.5}/><circle cx={0} cy={h/2} r={2.5} fill={isCT?'#0EA5E9':mu}/></g>}
    ...
    <div style={{display:'flex',flexDirection:'column',height:'100%',background:T.bg,overflow:'hidden'}}>
      ...
      <div style={{height:48,background:T.bgCard,borderBottom:`1px solid ${T.border}`,display:'flex',alignItems:'center',padding:'0 12px',gap:6,flexShrink:0}}>
        ...
        <div style={{display:'flex',gap:4,alignItems:'center',flexShrink:0}}>
          ...
          <button onClick={()=>{setZoom(0.88);setPan({x:30,y:10})}} style={{background:T.bgSurface,border:`1px solid ${T.border}`,borderRadius:6,width:28,height:28,cursor:'pointer',color:T.textMuted,fontSize:13,display:'flex',alignItems:'center',justifyContent:'center'}}>↩</button>
        ...
        <div style={{flex:1,position:'relative',overflow:'hidden',cursor:panning?'grabbing':'default'}}
          ...
          <svg ref={svgRef} style={{position:'absolute',inset:0,width:'100%',height:'100%',overflow:'visible'}} onMouseMove={onCvsMove} onMouseUp={onCvsUp}>
            <g transform={`translate(${pan.x},${pan.y}) scale(${zoom})`}>
              ...
              {nodes.map(node=><FlowNode key={node.id} node={node} selected={selNode===node.id} mode={mode} isCT={!!conn&&conn.nodeId!==node.id} onMD={e=>{startDrag(e,node.id);setSelNode(node.id)}} onPD={port=>startConn(node.id,port)} onPU={port=>finishConn(node.id,port)}/>)}
            </g>
          </svg>
          ...
          <div style={{position:'absolute',bottom:14,right:14,background:T.bgCard,border:`1px solid ${T.border}`,borderRadius:8,padding:'4px 10px',color:T.textFaint,fontSize:10.5}}>{nodes.length} nodos · {edges.length} conexiones</div>
        </div>

        {/* Editor panel — ALL inputs are direct children of FlowsPage, no sub-components */}
        <div style={{width:272,background:T.bgCard,borderLeft:`1px solid ${T.border}`,flexShrink:0,overflow:'hidden',display:'flex',flexDirection:'column'}}>
          {!selN ? (
            <div style={{padding:24,textAlign:'center',marginTop:60}}>
              <div style={{color:T.textFaint,fontSize:13,lineHeight:1.8}}>
                Hacé click en un nodo para editarlo.<br/>
                Arrastrá el <b style={{color:T.text}}>●</b> derecho al izquierdo para conectar.
              </div>
            </div>
          ) : (
            <div style={{display:'flex',flexDirection:'column',height:'100%'}}>
              ...
              {/* Fields — all direct in JSX, no helper components */}
              <div style={{flex:1,overflowY:'auto',padding:14}}>

                {selN.type==='message'&&<>
                  <label style={lbl}>Texto del mensaje</label>
                  <textarea style={{...base,resize:'vertical',lineHeight:1.5}} rows={4}
                ...
                {selN.type==='options'&&<>
                  ...
                  <button onClick={addO} style={{width:'100%',marginTop:4,background:meta.color+'15',border:`1px dashed ${meta.color}66`,borderRadius:8,padding:'7px',color:meta.color,fontSize:12,cursor:'pointer',fontWeight:600,display:'block'}}>
                    + Agregar botón
                  </button>
                </>}
                ...
                {selN.type==='input'&&<>
                  ...
                  <label style={lbl}>Guardar en variable</label>
                  <input style={base} placeholder="nombre, email..." value={selN.data.variable||''} onChange={e=>setF('variable',e.target.value)}/>
                </>}

                {selN.type==='delay'&&<>
                  <label style={lbl}>Espera: {selN.data.seconds||5}s</label>
                  <input type="range" min={1} max={120} style={{width:'100%',marginBottom:12}}
                    value={selN.data.seconds||5} onChange={e=>setF('seconds',+e.target.value)}/>
                  <label style={lbl}>Mensaje</label>
                  <input style={base} placeholder="Un momento..." value={selN.data.message||''} onChange={e=>setF('message',e.target.value)}/>
                </>}

                {selN.type==='agent'&&<>
                  <label style={lbl}>Mensaje de transferencia</label>
                  <textarea style={{...base,resize:'vertical',lineHeight:1.5}} rows={3}
              ...
              {selN.type!=='trigger'&&(
                <div style={{padding:'10px 14px',borderTop:`1px solid ${T.border}`,flexShrink:0}}>
                  <button onClick={()=>deleteNode(selN.id)} style={{width:'100%',background:`${T.danger}12`,border:`1px solid ${T.danger}33`,borderRadius:8,padding:'8px',color:T.danger,fontSize:12,cursor:'pointer',fontWeight:600}}>
                    🗑 Eliminar nodo
                  </button>
                </div>
              )}
