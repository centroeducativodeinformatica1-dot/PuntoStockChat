    ...
    <div style={{ display:'flex', height:'100vh', background:T.bg, fontFamily:'system-ui,sans-serif', overflow:'hidden', transition:'background 0.3s' }}>
      ...
      <div style={{ width:60, background:T.bgCard, borderRight:`1px solid ${T.border}`, display:'flex', flexDirection:'column', alignItems:'center', padding:'14px 0', gap:2, flexShrink:0, boxShadow:T.shadow }}>
        ...
        <button onClick={toggle} title={mode==='light'?'Modo oscuro':'Modo claro'}
          style={{ width:34, height:34, borderRadius:8, background:T.bgSurface, border:`1px solid ${T.border}`, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', marginBottom:8, transition:'all 0.15s' }}>
          <Icon n={mode==='light'?'moon':'sun'} size={16} color={T.textMuted}/>
        </button>

        {user?.photoURL
          ? <img src={user.photoURL} style={{ width:32, height:32, borderRadius:'50%', border:`2px solid ${T.primary}44`, cursor:'pointer' }} title={user.email}/>
          : <div style={{ width:32, height:32, borderRadius:'50%', background:T.primaryLight, display:'flex', alignItems:'center', justifyContent:'center' }}>
              <Icon n="user" size={15} color={T.primary}/>
            </div>
        }
