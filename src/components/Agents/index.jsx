    ...
    <div style={{ height:'100%', overflowY:'auto', padding:24, background:T.bg }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
        ...
        {isAdmin && (
          <button onClick={()=>setShowInvite(s=>!s)}
            style={{ background:T.primary, border:'none', borderRadius:10, padding:'9px 16px', color:'#fff', fontSize:13, fontWeight:600, cursor:'pointer', display:'flex', alignItems:'center', gap:7, boxShadow:`0 2px 8px ${T.primary}44` }}>
          ...
          <div style={{ display:'flex', gap:8, marginBottom:14 }}>
            {Object.entries(ROLES).map(([r,l])=>(
              <button key={r} onClick={()=>setInviteRole(r)}
                style={{ flex:1, padding:'9px', borderRadius:10, background:inviteRole===r?T.primaryLight:'transparent', border:`1px solid ${inviteRole===r?T.primary:T.border}`, color:inviteRole===r?T.primary:T.textMuted, fontSize:12.5, fontWeight:inviteRole===r?700:400, cursor:'pointer', transition:'all 0.15s' }}>
                {l}
              </button>
            ))}
          </div>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        {agents.map(agent=>{
          ...
          return (
            <div key={agent.id} style={{ background:T.bgCard, borderRadius:12, padding:'16px 18px', border:`1px solid ${T.border}`, display:'flex', alignItems:'center', gap:14, boxShadow:T.shadow }}>
