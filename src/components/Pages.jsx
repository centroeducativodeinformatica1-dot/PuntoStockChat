...
// ── Helpers definidos FUERA de los componentes para evitar re-renders ─────────
const inputStyle = (T) => ({
...
function Field({ label, children }) {
  return (
    <div style={{ marginBottom:14 }}>
      <div style={{ color:'#94A3B8', fontSize:10.5, fontWeight:700, letterSpacing:0.4, textTransform:'uppercase', marginBottom:5 }}>
        {label}
      </div>
      {children}
    </div>
...
function Toggle({ checked, onChange, primary }) {
  return (
    <div
      onClick={() => onChange(!checked)}
      style={{ width:38, height:22, borderRadius:11, background:checked?primary:'#CBD5E1', cursor:'pointer', position:'relative', transition:'all 0.2s', flexShrink:0 }}
    >
      ...
      <div style={{ display:'grid', gridTemplateColumns:'360px 1fr', gap:20 }}>
        <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                ...
                {COLORS.map(c => (
                  <div key={c} onClick={() => setPrimaryColor(c)}
                    style={{ width:30, height:30, borderRadius:'50%', background:c, cursor:'pointer',
                      border:primaryColor===c?`3px solid ${T.text}`:'3px solid transparent',
                      transition:'border 0.15s',
                      boxShadow:primaryColor===c?`0 0 0 2px ${c}66`:'none' }}/>
                ))}
              </div>
            </Field>
                ...
                {['left','right'].map(p => (
                  <button key={p} onClick={() => setPosition(p)}
                    style={{ flex:1, padding:'8px', borderRadius:10,
                      background:position===p?T.primaryLight:'transparent',
                      border:`1px solid ${position===p?T.primary:T.border}`,
                      color:position===p?T.primary:T.textMuted,
                      fontSize:12.5, fontWeight:position===p?700:400, cursor:'pointer', transition:'all 0.15s' }}>
          ...
          <div style={{ background:T.bgCard, borderRadius:14, padding:20, border:`1px solid ${T.border}`, boxShadow:T.shadow }}>
            ...
            {[
              ['Abrir automáticamente', autoOpen,     setAutoOpen],
              ['Mostrar en móvil',      showOnMobile, setShowOnMobile],
              ['Pedir nombre',          collectName,  setCollectName],
              ['Pedir email',           collectEmail, setCollectEmail],
            ].map(([label, val, setter]) => (
              <div key={label} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
                <span style={{ color:T.text, fontSize:13 }}>{label}</span>
                <Toggle checked={val} onChange={setter} primary={T.primary}/>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
          <div style={{ background:T.bgCard, borderRadius:14, border:`1px solid ${T.border}`, overflow:'hidden', boxShadow:T.shadow }}>
            ...
            <div style={{ height:300, background:'linear-gradient(135deg,#EFF6FF,#DBEAFE)', position:'relative' }}>
              ...
              <div style={{ position:'absolute', bottom:16, [position]:16, width:50, height:50, borderRadius:'50%', background:primaryColor, display:'flex', alignItems:'center', justifyContent:'center', boxShadow:`0 4px 16px ${primaryColor}55` }}>
                <Icon n="message" size={20} color="#fff"/>
              </div>
              <div style={{ position:'absolute', bottom:74, [position]:16, width:220, background:'#fff', borderRadius:14, boxShadow:'0 8px 30px rgba(0,0,0,0.15)', overflow:'hidden' }}>
                ...
                <div style={{ padding:10 }}>
                  <div style={{ background:`${primaryColor}12`, border:`1px solid ${primaryColor}22`, borderRadius:'8px 8px 8px 2px', padding:'7px 10px', fontSize:11, color:'#1e293b', maxWidth:170 }}>
                    {greeting||'¡Hola!'}
                  </div>
                </div>
              </div>
          ...
          <div style={{ background:T.bgCard, borderRadius:14, border:`1px solid ${T.border}`, boxShadow:T.shadow }}>
            ...
            <pre style={{ margin:0, padding:16, color:'#0EA5E9', fontSize:11, lineHeight:1.7, overflowX:'auto', whiteSpace:'pre-wrap', wordBreak:'break-word', background:T.bg }}>
              {scriptCode}
            </pre>
    ...
    <div style={{ height:'100%', overflowY:'auto', padding:24, maxWidth:560, background:T.bg }}>
      ...
      <div style={{ background:T.bgCard, borderRadius:14, padding:24, border:`1px solid ${T.border}`, marginBottom:16, boxShadow:T.shadow }}>
        ...
        <div style={{ marginBottom:20 }}>
          <div style={{ color:'#94A3B8', fontSize:10.5, fontWeight:700, letterSpacing:0.4, textTransform:'uppercase', marginBottom:5 }}>Email</div>
      ...
      <div style={{ background:T.bgCard, borderRadius:14, padding:20, border:`1px solid ${T.border}`, boxShadow:T.shadow }}>
        ...
        <div style={{ color:T.textMuted, fontSize:12.5, lineHeight:1.7 }}>
          ...
          Modo {mode==='light'?'claro 🌤':'oscuro 🌙'} activo
        </div>
      </div>
    </div>
  )
